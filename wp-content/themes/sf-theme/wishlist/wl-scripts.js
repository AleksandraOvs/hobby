            ...
            function (response) {
                if (response.success) {
                    if (response.data.status === 'added') {
                        ...
                        // если удаляем из вишлиста на странице вишлиста — удаляем карточку
                        btn.closest('.wishlist-item').fadeOut(300, function () { $(this).remove(); });
                    }
                }
            }
        );
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateTotal(row) {
        ...
        // Отображаем с форматированием WooCommerce
