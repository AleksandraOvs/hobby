...
add_action('wp_enqueue_scripts', function () {
    ...
    // передаем ajax_url в JS
    wp_localize_script('wl_scripts', 'wl_ajax', array(
        'ajax_url' => admin_url('admin-ajax.php')
    ));
});

// 1. Добавление кнопки «Добавить в избранное» на страницу товара или каталог
...
{
    if (!is_user_logged_in() || !isset($_POST['product_id'])) wp_send_json_error();
...
//2. Шорткод для вывода вишлиста

// Шорткод для вишлиста
add_shortcode('custom_wishlist', 'custom_wishlist_shortcode');

function custom_wishlist_shortcode()
{
    if (!is_user_logged_in()) {
        return '<p>Войдите, чтобы просматривать избранное</p>';
    }
            ...
            ?>
                ...
                <div class="cart-flex__row wishlist-item">
                    ...
                    <div class="cart-flex__col cart-flex__col--price">
                        ...
                        <span class="price" data-raw-price="<?php echo esc_attr($price); ?>">
                            <?php echo $price_html; ?>
                        </span>
                    </div>
